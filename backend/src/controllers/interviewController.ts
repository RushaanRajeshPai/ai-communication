...
export const processInterview = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const prompt = `
You are an expert interview coach. Analyze the following interview responses and provide detailed feedback.
...
- Average Rate of Speech: ${rateOfSpeech} words per minute
- Filler Words Used: ${fillerWordCount}
...
- Strings must be plain text without trailing explanations outside quotes.
...
try {
  ...
  console.warn("Failed to parse AI JSON, using fallback. Error:", e);
  feedbackData = {
